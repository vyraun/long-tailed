...
@register_criterion('average_focal_loss')
class AverageFocalLossCriterion(FairseqCriterion):
    ...
    def compute_loss(self, model, net_output, sample, reduce=True):
        ...
        pt = pt.view(pt.shape[0], -1, 3)
        lprobs = lprobs.view(lprobs.shape[0], -1, 3)

        pt = torch.sum(pt, dim=2)/3
        lprobs = torch.sum(lprobs, dim=2)/3
