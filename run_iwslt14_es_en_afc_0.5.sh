...
# Train on the data
SAVE_DIR=checkpoints/transformer_es_en_afc_0.5
mkdir -p $SAVE_DIR

tempdir=$(mktemp -d /tmp/XXXXX)
cp -r data-bin/iwslt14.tokenized.es-en $tempdir

CUDA_VISIBLE_DEVICES=1 fairseq-train $tempdir/iwslt14.tokenized.es-en \
  -a transformer_iwslt_de_en --optimizer adam --lr 0.0005 -s es -t en \
  --dropout 0.3 --max-tokens 4000 \
  --min-lr '1e-09' --lr-scheduler inverse_sqrt --weight-decay 0.0001 \
  --criterion focal_loss --gamma 1 --alpha 0.5 --max-update 50000 \
