...
# Train on the data
SAVE_DIR=checkpoints/transformer_de_en_final_fc1
mkdir -p $SAVE_DIR

tempdir=$(mktemp -d /tmp/XXXXX)
cp -r data-bin/iwslt14.tokenized.de-en $tempdir

CUDA_VISIBLE_DEVICES=3 fairseq-train $tempdir/iwslt14.tokenized.de-en \
  -a transformer_iwslt_de_en --optimizer adam --lr 0.0005 -s de -t en \
  --dropout 0.3 --max-tokens 4000 \
  --min-lr '1e-09' --lr-scheduler inverse_sqrt --weight-decay 0.0001 \
  --criterion label_smoothed_cross_entropy --label-smoothing 0.1 --max-update 50000 \
